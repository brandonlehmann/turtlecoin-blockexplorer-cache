...
const Self = function (opts) {
  ...
  if (mysql === null) {
    throw new Error('mysql/mariadb support not found, are you sure its installed?')
  }
...
Self.prototype._createDatabase = function () {
  return new Promise((resolve, reject) => {
    this.run('CREATE TABLE IF NOT EXISTS `transactions` (' +
      ...
      return this.run('ALTER TABLE `blocks` ADD INDEX (`timestamp`)', [])
    }).then(() => {
      return this.run('ALTER TABLE `transactions` ADD INDEX (`paymentId`)', [])
    }).then(() => {
      return this.run('ALTER TABLE `transactions` ADD INDEX (`blockHash`)', [])
    }).then(() => {
